...
class AnimatedBorderPainter extends CustomPainter {
  ...
  Path extractPath(Path originalPath, double length, {double offset = 0}) {
    ...
    for (final metric in originalPath.computeMetrics()) {
      if (currentLength + metric.length > offset) {
        ...
        path.addPath(metric.extractPath(start, start + extractLength), Offset.zero);
        break;
      }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final firstHalfLength = (currentLength > halfLength) ? halfLength : currentLength;
    final secondHalfLength = (currentLength < halfLength) ? currentLength - halfLength : currentLength;
